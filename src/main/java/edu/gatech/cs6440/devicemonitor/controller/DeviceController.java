...
public class DeviceController {
	...
	@GetMapping("device/{udi}/name")
	@ResponseBody
	public ResponseEntity<?> getDeviceNameByUDI(@PathVariable String udi, @RequestHeader("token") String token) {
		if (userService.auth(token) == null) {
			return new ResponseEntity<String>("Unauthorized", HttpStatus.UNAUTHORIZED);
		}
		DevicePojo device = deviceService.findDevicePojoByUDI(udi);
		return new ResponseEntity<>(device.getName(), HttpStatus.OK);
	}
